...
public class ImportExampleJavaMapAsMapWorker extends FWorker<Void, NonTerminal> {
    ...
    public ImportExampleJavaMapAsMapWorker(AppContext ac, WorldContext wc, String name) {
        super(ac, wc, name);
    }
    ...
    protected NonTerminal background(Void gathered) throws Exception {
        ...
        NonTerminal M = new JavaObjectMapAwareExtractor(m).extract();
        M.putSysMeta("time", DateUtil.toLongString(System.currentTimeMillis()));
        M.putSysMeta("source", "Java Map");
        Mcontext.put(K, M);
        select(
            new SelectRequest()
                .setContext(C)
    ...
    public String getActionVerb() {
        return "importing example map as a map";
    }
}
