...
public class EditContextPanel extends FPanel {
    ...
    public EditContextPanel(final AppContext ac) {
        this.ac = ac;
        ...
        ac.addEditContextChangedListener(new ChangeListener() {
            @Override
    ...
    private void updateEditContexts() {
        ViewPanel pnlView = ac.getSelectedViewPanel();
        nRoot.removeAllChildren();
        if(pnlView != null) {
            SelectionContext[] Cs = pnlView.getEditContextContexts();
            if(Cs != null) {
                for(SelectionContext C : Cs) {
                    nRoot.add(new NodeEditContext(C));
                }
            }
        }
    ...
    private class NodeEditContext extends NodeBase {
        private SelectionContext C;

        public NodeEditContext(SelectionContext C) {
            this.C = C;
        }
        ...
        public Icon getIcon(boolean expanded) {
            return ImageLib.get(FinioImageModel.EDIT_ACTIVE);
        }
